...
import { Logger } from '../../context/errors'
...
export function upsertOrganizationFromClerkService(
    ...
    return async (input: { clerkOrganizationId: string; name: string; email: string; clerkUserId: string }) => {
        ...
        if (!user) {
            Logger.warn('User not found when upserting organization, creating user from Clerk', { clerkUserId })
