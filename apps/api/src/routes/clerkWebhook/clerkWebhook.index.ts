...
export const clerkWebhookRouter = new OpenAPIHono<{ Bindings: CloudflareBindings }>().openapi(
	clerkWebhookRoute,
	async (c) => {
			...
			switch (validatedEvent.type) {
				...
				case 'organizationMembership.updated': {
					...
					try {
						...
						const action =
							validatedEvent.type === 'organizationMembership.created' ? 'created' : 'updated'
				...
				default:
					Logger.error('Unknown event type received', {
						type: (validatedEvent as { type: string }).type,
					})
					return c.json(
						{
							success: false,
